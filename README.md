...
| `schema`      | `S.t<'value>` | A schema created with **[Sury](https://github.com/DZakh/sury)**. It's used for coercion and parsing. For bool schemas coerces `"0", "1", "true", "false", "t", "f"` to boolean values. For int, float and bigint schemas coerces string to number. For other non-string schemas the value is coerced using `JSON.parse` before being validated. |
